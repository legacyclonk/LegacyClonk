		...
		for (;;)
		{
			...
			case AB_LOCALN_R: case AB_LOCALN_V:
				...
				if (pCurCtx->Func->Owner->Def != pCurCtx->Obj->Def)
				{
					...
					{
						ThrowExecError(pCPos, FormatString("can't access local variable \"%s\" after ChangeDef!", localName).getData());
					}
			...
			case AB_ConcatIt: // ..=
			{
				...
				switch (pPar1->GetType())
				{
					...
					default:
					{
						...
						if (!par1String)
						{
							ThrowExecError(pCPos, FormatString("operator \"%s\" left side: can not convert \"%s\" to \"string\", \"array\" or \"map\"!", operatorName, GetC4VName(pPar1->GetType())).getData());
						}
						auto par2String = pPar2->toString();
						if (!par2String)
						{
							ThrowExecError(pCPos, FormatString("operator \"%s\" right side: can not convert \"%s\" to \"string\"!", operatorName, GetC4VName(pPar2->GetType())).getData());
						}
			...
			case AB_ARRAYA_R: case AB_ARRAYA_V:
			{
				...
				if (Container.ConvertTo(C4V_String))
				{
					if (!Index.ConvertTo(C4V_Int))
						ThrowExecError(pCPos, FormatString("indexed string access: index of type %s, int expected!", Index.GetTypeName()).getData());
					...
					ThrowExecError(pCPos, FormatString("indexed access: can't access %s by index!", Container.GetTypeName()).getData());
			}
			...
			case AB_MAPA_R: case AB_MAPA_V:
			{
				...
				{
					ThrowExecError(pCPos, FormatString("map access with .: map expected, but got \"%s\"!", GetC4VName(Map.GetType())).getData());
				}
			...
			case AB_ARRAY_APPEND:
			{
				...
				if (!Array.ConvertTo(C4V_Array) || Array.GetType() != C4V_Array)
					ThrowExecError(pCPos, FormatString("array append accesss: can't access %s as an array!", Array.GetType() == C4V_Any ? "nil" : Array.GetTypeName()).getData());
			...
			case AB_FUNC:
			{
				...
				{
					ThrowExecError(pCPos, FormatString("Insufficient access level for function \"%s\"!", pFunc->Name).getData());
				}
			...
			case AB_VAR_R: case AB_VAR_V:
				if (!pCurVal->ConvertTo(C4V_Int))
					ThrowExecError(pCPos, FormatString("Var: index of type %s, int expected!", pCurVal->GetTypeName()).getData());
			...
			case AB_PAR_R: case AB_PAR_V:
				if (!pCurVal->ConvertTo(C4V_Int))
					ThrowExecError(pCPos, FormatString("Par: index of type %s, int expected!", pCurVal->GetTypeName()).getData());
			...
			case AB_FOREACH_NEXT:
			{
				...
				if (!iItem)
				{
					if (!pCurVal[-1].ConvertTo(C4V_Array))
						ThrowExecError(pCPos, FormatString("for: array expected, but got %s!", pCurVal[-1].GetTypeName()).getData());
					if (!pCurVal[-1]._getArray())
						ThrowExecError(pCPos, FormatString("for: array expected, but got nil!").getData());
				}
			...
			case AB_FOREACH_MAP_NEXT:
			{
				...
				if (!iterator)
				{
					if (!pCurVal[-2].ConvertTo(C4V_Map))
						ThrowExecError(pCPos, FormatString("for: map expected, but got %s!", pCurVal[-1].GetTypeName()).getData());
					if (!pCurVal[-2]._getMap())
						ThrowExecError(pCPos, FormatString("for: map expected, but got nil!").getData());
				}
