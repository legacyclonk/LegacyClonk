...
{
	// max alpha
	uint32_t dwAlpha = std::max<uint32_t>(rdwClr >> 24 & 255, 0xff) << 24;
	rdwClr &= 0xffffff;
	...
	if (iLightness < 16575)
	{
		...
		// otherwise, lighten
		rdwClr = (std::min<uint32_t>(r + iInc, 255) << 16) | (std::min<uint32_t>(g + iInc, 255) << 8) | std::min<uint32_t>(b + iInc, 255);
	}
...
void Element::DrawVBar(C4FacetEx &cgo, DynBarFacet &rFacets)
{
	...
	{
		int32_t h2 = (std::min)(h, rcBounds.Hgt - 5 - iY); rFacets.fctMiddle.Hgt = h2;
...
void Element::DrawHBarByVGfx(C4FacetEx &cgo, DynBarFacet &rFacets)
{
	...
	{
		int32_t h2 = (std::min)(h, rcBounds.Wdt - 5 - iY); rFacets.fctMiddle.Hgt = h2;
...
void CMouse::Draw(C4FacetEx &cgo, bool fDrawToolTip)
{
	...
	GfxR->fctMouseCursor.DrawT(cgo.Surface, static_cast<int>(x * scale + iOffsetX), static_cast<int>(y * scale + iOffsetY), 0, 0, &transform, true);
	if (Game.MouseControl.IsHelp())
		GfxR->fctMouseCursor.DrawT(cgo.Surface, static_cast<int>(x * scale + iOffsetX + 5), static_cast<int>(y * scale + iOffsetY - 5), 29, 0, &transform, true);
	// ToolTip
