...
C4Player *C4PlayerList::Join(const char *szFilename, bool fScenarioInit, int iAtClient, const char *szAtClientName, C4PlayerInfo *pInfo)
{
	...
	if (1) // replay needs to check, too!
		...
		{
			Log(LoadResStr(C4ResStrTableKey::IDS_PRC_TOOMANYPLRS, Game.Parameters.MaxPlayers));
			return nullptr;
		}
	...
	{
		Log(LoadResStr(C4ResStrTableKey::IDS_PRC_PLRFILEINUSE)); return nullptr;
	}
	...
	{
		Remove(pPlr, false, false); Log(LoadResStr(C4ResStrTableKey::IDS_PRC_JOINFAIL)); return nullptr;
	}
...
bool C4PlayerList::RemoveAtClient(int iClient, bool fDisconnect)
{
	...
	{
		// Log
		Log(LoadResStr(C4ResStrTableKey::IDS_PRC_REMOVEPLR, pPlr->GetName()));
		// Remove
...
{
	// message
	Log(LoadResStr(C4ResStrTableKey::IDS_PRC_SYNCPLRS));
	bool fSuccess = true;
	// check all players
	for (C4Player *pPlr = First; pPlr; pPlr = pPlr->Next)
