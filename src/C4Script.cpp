...
static C4String *FnGetTaggedPlayerName(C4Player &player)
{
	...
	sprintf(szFnFormatBuf, "<c %x>%s</c>", dwClr & 0xffffff, player.GetName());
...
static C4ValueInt FnGetPlrJumpAndRunControl(Required<C4Player *, -1> plr)
{
	return plr->ControlStyle;
}

static C4ValueInt FnGetPlrViewMode(Required<C4Player *, -1> plr)
{
	return plr->ViewMode;
}
...
static C4Value FnDefinitionCall(C4AulContext *cthr,
	RequiredNonZero<C4ID> idID, C4String &szFunction,
	C4Value par0, C4Value par1, C4Value par2, C4Value par3, C4Value par4,
	C4Value par5, C4Value par6, C4Value par7)
{
	// Make failsafe
...
private:
	...
	C4Value ExecHelper(C4AulContext *context, const C4Value pars[], std::index_sequence<indices...>) const
	{
		...
		const auto callHelper = [=, this]
		{
			if constexpr (NeedsContext)
			{
				return func(context, ArgumentConverter<Pars>::Convert(context, pars[indices])...);
			}
			else
			{
				return func(ArgumentConverter<Pars>::Convert(context, pars[indices])...);
			}
		};
