...
bool C4FontLoader::InitFont(CStdFont &rFont, const char *szFontName, FontType eType, int32_t iSize, C4GroupSet *pGfxGroups, bool fDoShadow)
{
	...
	{
		LogFatal(LoadResStr(C4ResStrTableKey::IDS_ERR_INITFONTS));
		return false;
	}
	// get font to load
	...
	default: LogFatal(LoadResStr(C4ResStrTableKey::IDS_ERR_INITFONTS)); return false; // invalid call
	}
	// font not assigned?
	if (!*szFontString)
	{
		// invalid call or spec
		LogFatal(LoadResStr(C4ResStrTableKey::IDS_ERR_INITFONTS)); return false;
	}
	// get font name
	...
	if (SEqualNoCase(szExt, "png") || SEqualNoCase(szExt, "bmp"))
	{
		...
		if (!rFont.IsSameAsID(FontFaceName, iGrpId, iIndent))
		{
			...
			if (rFont.IsInitialized())
			{
				// reloading
				rFont.Clear();
				LogF(LoadResStr(C4ResStrTableKey::IDS_PRC_UPDATEFONT), FontFaceName, iIndent, 0);
			}
			C4Surface sfc;
	...
	else
	{
		...
		if (!rFont.IsSameAs(FontFaceName, iDefFontSize, dwDefWeight))
		{
			...
			if (rFont.IsInitialized())
			{
				// reloading
				rFont.Clear();
				LogF(LoadResStr(C4ResStrTableKey::IDS_PRC_UPDATEFONT), FontFaceName, iDefFontSize, dwDefWeight);
			}
