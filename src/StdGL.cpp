	...
	if (DDrawCfg.ClipManuallyE)
		...
		if (x1 < ClipX1)
			...
			if (x1 > ClipX2)
				...
				{ y2-=(y2-y1)*(x2-(float)ClipX2)/(x2-x1); x2=(float)ClipX2; } // clip right
		// sort top/bottom
		...
		if (y1 < ClipY1)
			if (y2 < ClipY1)
				...
				{ x1+=(x2-x1)*((float)ClipY1-y1)/(y2-y1); y1=(float)ClipY1; } // clip top
		else if (y2 > ClipY2)
			if (y1 > ClipY2)
				return; // bottom out
			else
				{ x2-=(x2-x1)*(y2-(float)ClipY2)/(y2-y1); y2=(float)ClipY2; } // clip bottom
		}
