...
{
	return obj ? *obj->Section : Game.MainSection;
}
...
void C4Object::SetOCF()
{
	...
	{
		LogF("Warning: contained in wild object %p!", Contained.Denumerated());
	}
	else if (Contained && !Contained->Status)
	{
		LogF("Warning: contained in deleted object %p (%s)!", Contained.Denumerated(), Contained->GetName());
	}
#endif
	if (Contained)
...
void C4Object::DrawTopFace(C4FacetEx &cgo, int32_t iByPlayer, DrawMode eDrawMode)
{
	...
	if (OCF & OCF_CrewMember) if ((Config.Graphics.ShowCrewNames || Config.Graphics.ShowCrewCNames) && (!Game.MainSection.C4S.Head.Film || !Game.MainSection.C4S.Head.Replay)) if (!eDrawMode)
...
C4Object *C4Object::ComposeContents(C4ID id)
{
	...
	for (cnt = 0; c_id = NeededComponents.GetID(cnt); cnt++)
		...
		{
			Needs.AppendFormat("|%ix %s", NeededComponents.GetCount(cnt) - Contents.ObjectCount(c_id), Game.Defs.ID2Def(c_id) ? Game.Defs.ID2Def(c_id)->GetName() : C4IdText(c_id));
...
void C4Object::DrawSelectMark(C4FacetEx &cgo)
{
	...
	if (Game.MainSection.C4S.Head.Film && Game.MainSection.C4S.Head.Replay) return;
	// target pos (parallax)
	...
	for (cnt = 0; idComponent = NeededComponents.GetID(cnt); cnt++)
	{
		if (NeededComponents.GetCount(cnt) != 0)
		{
			...
			if (ncnt > 0)
			{
				...
				if ((pComponent = Game.Defs.ID2Def(idComponent)))
