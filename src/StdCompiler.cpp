...
void StdCompilerINIRead::CreateNameTree()
{
	...
	while (*pPos)
	{
		...
		if (fSection || std::isalpha(static_cast<unsigned char>(*pPos)))
		{
			...
			if (*pPos != (fSection ? ']' : '='))
				// Warn, ignore
				Warn(std::isprint(static_cast<unsigned char>(*pPos)) ? "Unexpected character ('%c'): %s ignored" : "Unexpected character ('0x%02x'): %s ignored", unsigned(*pPos), fSection ? "section" : "value");
			else
			{
				pPos++;
				// Create new node
