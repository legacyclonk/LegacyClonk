...
public:
	...
	SoundChannel *CreateSoundChannel(const SoundFile *const sound, bool loop) { return new SoundChannelSdl{static_cast<const SoundFileSdl *>(sound), loop}; }
	...
	virtual SoundFile *CreateSoundFile(const void *buf, std::size_t size) { return new SoundFileSdl{buf, size}; }
};
