...
bool C4EditCursor::UpdateStatusBar()
{
	...
	case C4CNS_ModeDraw:
		//text = FormatString("%i/%i (%s)", X, Y, (section.MatValid(section.Landscape.GetMat(X, Y)) ? section.Material.Map[section.Landscape.GetMat(X, Y)].Name : LoadResStr("IDS_CNS_NOTHING")));
