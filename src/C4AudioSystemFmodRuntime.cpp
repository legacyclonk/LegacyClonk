...
#include <variant>
	...
	#define ErrorString std::to_string(GetLastError())
	#ifndef _WIN64
	...
	#define ErrorString std::string{dlerror()}
	#define F_API
#endif
...
Library::Library(const std::string &path) : lib{LoadLibraryA(path.c_str())}
{
	if (!lib)
	{
		throw std::runtime_error{"Could not LoadLibrary(" + path + "): " + ErrorString};
	}
}
...
void *Library::LoadSymbolInternal(const std::string &name)
{
	...
	{
		throw std::runtime_error{"Could not GetProcAddress(" + name + "): " + std::to_string(GetLastError())};
	}
	else
	{
