...
global func PlaceInMaterial(id def, int material, int retries) { // C4ID id, int iMaterial, [int iRetries]
  // Gültiger Materialindex? (-1 ist Sky)
  if (!Inside(material, 0, 127)) return nil;
  ...
  for (var retry_num = 0; retry_num < retries; ++retry_num) {
    ...
    if (GetMaterial(x, y) == material)
      return CreateObject(def, x, y, NO_OWNER);
  }
  return nil;
}
...
global func SetSpeed(int x_dir, int y_dir, object obj) { // int iXDir, int iYDir, [C4Object *pObj]
  ...
  return SetXDir(x_dir, obj) && SetYDir(y_dir, obj);
}
...
global func SetVertexXY(int index, int x, int y, object obj) { // int iIndex, int iXPos, int iYPos, [C4Object *pObj]
  ...
  // Vertices setzen
  return SetVertex(index, 0, x, obj) && SetVertex(index, 1, y, obj);
}
...
global func GetRGBaValue(int val, int sel) {
  return val >> ((3 - sel) * 8) & 0xFF;
}

// Abwärtskompatibilität
global func GetRGBValue(int val, int sel) {
...
global func FindDefinition(id idDef) {
  return GetDefCoreVal("id", "DefCore", idDef) == idDef;
}
...
global func LaunchRain(int x, int mat, int wdt, int lev) {
  var obj = CreateObject(FXP1, x - GetX(), -GetY(), NO_OWNER);
  if (!obj) return nil;
...
global func LaunchVolcano(int x, int y, int strength, string mat) {
  var obj = CreateObject(FXV1, x - GetX(), y - GetY(), NO_OWNER);
  if (!obj) return false;
  if (!strength) strength = BoundBy(15 * LandscapeHeight() / 500 + Random(10), 10, 60);

  obj->Activate(x, y || LandscapeHeight() - 1, strength, Material(mat || "Lava"));
  return true;
}
...
global func CastObjectsX(iddef, am, lev, x, y, angs, angw, callback) {
  ...
  for(var i = 0; i < am; ++i) {
    ...
    SetR(Random(360), obj = CreateObject(iddef, x, y, NO_OWNER));
...
global func FindConstructionSiteX(id idDef, &iVarX, &iVarY)
{
  ...
  if (!FindConstructionSite(idDef, 0, 1)) return false;
...
global func Visible(int iPlr, object pObj)
{
  ...
  if (GetOwner(pObj) == iPlr) if (iVisible & VIS_Owner) return true;
  // Objekt gehört einem Spieler, der iPlr feindlich gesonnen ist
  else if (Hostile(GetOwner(pObj), iPlr)) if (iVisible & VIS_Enemies) return true;
  // Objekt gehört einem Spieler, der iPlr freundlich gesonnen ist
  else if (iVisible & VIS_Allies) return true;
...
global func HSLa(int h, int s, int l, int a) { return HSL2RGB(RGB(h, s, l)) | (a & 0xFF) << 24; }
